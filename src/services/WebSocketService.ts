...
export class WebSocketService {
  ...
  private clients: Map<string, { ws: any; subscriptions: string[] }> = new Map(); // 使用 any 以避免类型冲突

  constructor(server: any) { // Express/HTTP server instance
  ...
  private setupEventHandlers(): void {
    this.wss.on('connection', (ws: any) => { // 使用 any 类型以避免类型冲突
      ...
      this.sendMessage(ws, { type: 'ping', source: 'server', data: { message: 'Connected to azhot WebSocket server' } });
    });
  }
  ...
  private async handleRequest(clientId: string, platform: string): Promise<void> {
    const client = this.clients.get(clientId);
    if (!client) return;
    ...
    if (platform === 'all') {
      ...
      this.sendMessage(client.ws, { type: 'request', source: platform, data: platforms });
    } else {
  ...
  public async broadcastToPlatform(platform: string, data: any): Promise<void> {
    for (const [clientId, client] of this.clients) {
      if (client.subscriptions.includes(platform) || client.subscriptions.includes('all')) {
        this.sendMessage(client.ws, { type: 'request', source: platform, data });
      }
    }
  }

  private sendMessage(ws: any, message: WebSocketMessage): void { // 使用 any 类型以避免类型冲突
    if (ws.readyState === ws.OPEN) {
      ws.send(JSON.stringify(message));
    }
  }

  private sendError(ws: any, errorMessage: string): void { // 使用 any 类型以避免类型冲突
    if (ws.readyState === ws.OPEN) {
      ws.send(JSON.stringify({ type: 'error', source: 'server', data: { error: errorMessage } }));
    }
  }
