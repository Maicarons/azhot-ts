...
async function fetchSouhuPage(page) {
  ...
  if (!response.ok) {
    throw new Error(
      `fetch page ${page}: HTTP error! status: ${response.status}`,
    );
  }
    ...
    try {
      ...
      if (fetchErrors.length > 0) {
        ...
        if (wordList.length === 0) {
          ...
          throw new Error(
            `获取数据失败: ${fetchErrors.map((e) => e.message).join(", ")}`,
          );
        }
        // 如果获取到部分数据，继续处理但记录错误
      ...
      for (
        let index = 0;
        index < Math.min(wordList.length, maxItems);
        index++
      ) {
