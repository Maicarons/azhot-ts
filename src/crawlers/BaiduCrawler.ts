...
export class BaiduCrawler extends BaseCrawler {
  constructor() {
    super(
      "baidu",
      "百度",
      "https://www.baidu.com/favicon.ico", // 根据Go代码中的图标URL更新
  ...
  async crawl(): Promise<HotItem[]> {
    try {
      ...
      // 使用正则表达式匹配热搜标题
      const pattern =
        /<div\s+class=\"c-single-text-ellipsis\">([\s\S]*?)<\/div>/g;
      const matches = [];
      let match;
      ...
      while ((match = pattern.exec(html)) !== null) {
        ...
        for (let i = 0; i < textContent.length; i++) {
          ...
          if (
            (charCode >= 32 && charCode <= 126) || // ASCII 可见字符
            ...
            if (
              charCode < 127 ||
              (charCode > 0x007f && charCode < 0x00a0) ||
              charCode > 0x009f
            ) {
      ...
      // 构造热搜项目数组
      const hotItems: HotItem[] = matches.map((title, index) => ({
        index: index + 1,
        title: title,
