...
export class DouYinCrawler extends BaseCrawler {
    constructor() {
        super('douyin', 'douyin', 'https://lf1-cdn-tos.bytegoofy.com/goofy/ies/douyin_web/public/favicon.ico', // 根据Go代码中的图标URL
    ...
    async crawl() {
        try {
            ...
            for (let index = 0; index < data.word_list.length; index++) {
                ...
                const hotValue = item.hot_value > 0 ? `${(item.hot_value / 10000).toFixed(2)}万` : '';
                items.push({
                    index: index + 1,
                    title: item.word,
