...
export class SougouCrawler extends BaseCrawler {
    constructor() {
        super('sougou', '搜狗', 'https://www.sogou.com/favicon.ico', 'https://www.sogou.com/web?query=%E6%90%9C%E7%8B%97%E7%83%AD%E6%90%9C');
    }
    async crawl() {
        try {
            // 创建带超时的请求
            ...
            const pattern = /<span [^>]*>[\s\S]*?<p>\s*<a href="([^"]+)"[^>]*>(.*?)<\/a>\s*<\/p>[\s\S]*?<\/span>\s*<span class="hot-rank-right">(.*?)<\/span>/gi;
            ...
            for (let index = 0; index < Math.min(matches.length, maxItems); index++) {
                ...
                if (item.length >= 4) {
                    ...
                    const parsedHotValue = parseFloat(hotValueStr.replace(/[^\d.-]/g, ''));
