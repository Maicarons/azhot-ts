...
// 检查平台是否已实现
export function isPlatformImplemented(platform) {
...
// 获取所有已实现平台的配置
export function getImplementedPlatforms() {
  return IMPLEMENTED_PLATFORMS.map(
    (platform) => PLATFORM_CONFIGS[platform],
  ).filter((config) => config !== undefined);
}
// 获取所有平台的配置
export function getAllPlatforms() {
  return ALL_PLATFORMS.map((platform) => PLATFORM_CONFIGS[platform]).filter(
    (config) => config !== undefined,
  );
}
// API响应状态码
export var API_STATUS;
(function (API_STATUS) {
  API_STATUS[(API_STATUS["SUCCESS"] = 200)] = "SUCCESS";
  API_STATUS[(API_STATUS["ERROR"] = 500)] = "ERROR";
  API_STATUS[(API_STATUS["NOT_FOUND"] = 404)] = "NOT_FOUND";
